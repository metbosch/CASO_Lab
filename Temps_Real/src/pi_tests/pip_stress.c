...
int main(void)
{
	...
	*minimum_priority = sched_get_priority_min(policy);
	...
	init_shared_pthread_mutex(resource, PTHREAD_PRIO_PROTECT, policy);
...
void Pthread_mutexattr_setprioceiling(pthread_mutexattr_t *attr, int prio)
{
	int err;
	err = pthread_mutexattr_setprioceiling(attr, prio);
	if (err) {
		err_msg("%s: pthread_mutexattr_setprioceiling(): ", __func__);
		err_exit(err, NULL);
	}
}
...
void init_shared_pthread_mutex(pthread_mutex_t *mutex, int protocol, int policy)
{
	...
	Pthread_mutexattr_setprioceiling(&attr, prio_min+10);
